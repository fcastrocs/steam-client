...
export default class Steam extends Connection {
    ...
    login(options) {
        if (this.connectionDestroyed)
            throw new SteamClientError("NotSteamConnection");
        ...
        return new Promise((resolve, reject) => {
            ...
            this.once("CMsgClientLicenseList", async (body) => {
                ...
                if (!packagesInfo.length)
                    return checkCanResolve("CMsgClientLicenseList");
                // get all appids from
                const appIds = [];
    ...
    getPackagesInfo(packageIds) {
        ...
        return new Promise((resolve) => {
            const packagesInfo = [];
            this.on("CMsgClientPICSProductInfoResponse", (res) => {
                if (res.packages) {
