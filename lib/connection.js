...
class Connection extends EventEmitter {
    ...
    jobidTargets = new Map();
    protoResponses = new Map();
    heartBeatId;
    ...
    sendUnified(serviceName, method, payload) {
     ...
     */
    buildMsgHdrProtoBuf(EMsg, unifiedMessage) {
    ...
    decodeData(data) {
        ...
        if (isProto) {
            ...
            if (body.jobidSource && !JOB_NONE.equals(body.jobidSource)) {
                const key = (Language.EMsgMap.get(EMsg) + "Response");
        ...
        if (EMsg === Language.EMsg.ServiceMethod)
            return;
